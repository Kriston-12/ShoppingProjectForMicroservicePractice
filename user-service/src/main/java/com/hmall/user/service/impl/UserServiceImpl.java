...
//        return vo;
//
// 上面的代码莫名其貌有问题，下面是直接加的
//        User user = userMapper.selectOne(new QueryWrapper<User>().eq("username", "Jack"));
        String token = jwtTool.createToken(Long.valueOf(1), jwtProperties.getTokenTTL());
        UserLoginVO vo = new UserLoginVO();
        vo.setUserId(Long.valueOf(1));
        vo.setUsername("Jack");
        vo.setToken(token);
        return vo;
    }
